...
async function choiceLoop(thisStudent) {
  ...
  if (thisStudent.clubPreferences) {
    for (
      ...
      const allMinSlotsFilled =
        choiceClubObject.minSlots9 == 0 &&
        choiceClubObject.minSlots10 == 0 &&
        choiceClubObject.minSlots11 == 0 &&
        choiceClubObject.minSlots12 == 0;
      // ** CHECK IF ALL GRADE SLOTS ARE SATISFIED AND IF SO THEN LET THEM IN IF THERE ARE SLOTS ******
      // console.log(gradeSlotsAvailable)
      if (
        (studentsInSameGrade < gradeSlotsAvailable || allMinSlotsFilled) &&
        maxSlotsAvailable - studentsInClub > 0 &&
...
async function getClubById(id) {
  ...
  const response = await fetch(
    `http://${serverAddress}:3000/getClubById?club=${id}`
  );
...
async function getRandomStudentOrder() {
  // **NEED TO CHANGE THIS TO FIND A RANDOM STUDENT WHO DOES NOT ALREADY HAVE A CLUBID **
  const students = await getStudents();
  const studentsWithoutOrder = students.filter(
    (student) => student.clubId === null
  );
