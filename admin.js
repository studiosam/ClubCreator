...
async function getAllApprovedClubs() {
  ...
  if (filteredClubs.length === 0) {
    ...
    try {
      await Promise.all(
        filteredClubs.map(async (club) => {
          ...
          console.log(currentCoSponsorsArray);
    ...
    <a class="cover-card-text" href="http://127.0.0.1:5500/club-info.html?club-id=${club.clubId}><p class="roboto uk-text-bold" id="clubName">${club.clubName
            }</p></a>
    ...
    <div class="uk-card-body">
      ...
      <span>Still Needed: </span><input name="requiredCoSponsors" id = "clubId${club.clubId
            }-coSponsorsRequired" class = "uk-input uk-width-1-2" type="number" value="${club.coSponsorsNeeded - currentCoSponsors
            }">
      </div>
    </div>
